...
export default function ProjectsPage() {
  ...
  // API-ඒකෙන් load කළ projects list

  const [loading, setLoading] = useState(true);
  // Page load-ඒකේ spinner

  const [error, setError] = useState(false);
  // Fetch fail වුනොත් error message

  // ── Category change වෙද්දී API call ─────────────────────────────────────
  // activeCategory වෙනස් වෙන හැම වතාවකම API-ඒකෙන් data ගන්නවා
  useEffect(() => {
    const fetchProjects = async () => {
      try {
        ...
        // category "All" නම් param නොදෙන — ඔක්කොම projects ගන්නවා
        // category "Residential" නම් ?category=Residential → filtered
  ...
  // [activeCategory] — category button click කළාම re-fetch වෙනවා

  // ── filtered projects — category-ඒකට අනුව ─────────────────────────────
  // Server-side filtering use කරන නිසා API-ඒකෙන් ඒ category-ඒකේ projects විතරක් එනවා
  // filtered count show කරන්නත් projects.length use කරනවා
        ...
        <div className="container mx-auto px-6 py-4 flex items-center gap-3 overflow-x-auto scrollbar-none">
          {categories.map((cat) => (
            <button
              ...
              // ✅ click කළාම setActiveCategory → useEffect trigger → API call
          ...
          <span className="ml-auto shrink-0 text-muted-foreground text-sm">
            {/* Loading-ඒකේදී count hide කරනවා */}
          ...
          <AnimatePresence mode="wait">
            ...
            >
              {projects.map((project, i) => (
                <motion.div
                  key={project.id}
                  // ✅ key-ඒකට project.id — title duplicate වෙනවා නම් safe
                ...
                >
                  ...
                  <div className={`relative overflow-hidden ${project.featured ? "h-72" : "h-56"}`}>
                    {/* ✅ /api/files/ — file serving route-ඒකෙන් image load කරනවා */}
                    <Image
